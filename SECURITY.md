...
5. **Téléchargement attachments / enrichissement BDC** : `docApi.getAccessToken({ readOnly: true })` → REST `?auth=` (jeton court, scoped doc / utilisateur) pour devis et lecture table `BDC`. **Pas** de `VITE_GRIST_API_KEY`.
