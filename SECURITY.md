...
4. **Écriture métier** : interdite dans les hooks PA/BDC/Missions (pas de create/update `Realise`, etc.). **Exception bornée** : create **uniquement** sur la table `Retours` via `grist.getTable` + garde [`writeTableAllowlist.ts`](src/security/writeTableAllowlist.ts) (pas d’update/delete côté widget). ACL Grist Create vs Update sur `Retours` = HITL (voir [`docs/fonctionnel/feedback/`](docs/fonctionnel/feedback/)).
