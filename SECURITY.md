...
4. **Écriture métier** : uniquement via `grist.getTable` + garde [`writeTableAllowlist.ts`](src/security/writeTableAllowlist.ts) — **pas** de REST avec clé API, **pas** de delete widget. Exceptions : `Retours` (create feedback) ; `Missions` (create + update patch Nom / Produit / Statut via drawer) ; `Missions_enfants` (create + update via drawer prestation — `Titre_de_la_prestation`, intervenant, jours, statut, date de début ; type Freelance figé à la création ; pas de delete) ; `Realise` (create + update déclaration CRA freelance — `Nb_jours`, `Taches_realisees`, période, rattachements ; update revue équipe — `Nb_jours`, `Taches_realisees`, `BDC_cible` ; pas de delete ni `Calcul_TTC`) ; `Acl_profil` (**create only**, fiche session auto si absente — `E_mail` = compte connecté ; pas d’update/delete) ; `Droits_pages` (**update only**, page Admin — cases `Page_*` ; `Page_accueil` forcé à vrai ; pas de create/delete de rôles). Tout le reste (delete) reste interdit. ACL Grist Create/Update = HITL (feedback + missions + Realise déclaration/revue + Acl_profil + droits pages). Access Rules `Realise` (mur freelance / département) : **fait** (#47 / #70) — voir [`docs/fonctionnel/roles/access-rules.md`](docs/fonctionnel/roles/access-rules.md).
5. **Téléchargement attachments / lecture BDC / Realise / Equipe (auteur feedback)** : `docApi.getAccessToken({ readOnly: true })` → REST `?auth=` (jeton **court**, scoped **document + utilisateur** connecté) quand pertinent ; `Equipe` aussi via `fetchTable` allowlisté pour le select auteur. **Pas** de `VITE_GRIST_API_KEY`. Le jeton n’existe que pendant une session iframe Grist authentifiée — il n’est **pas** publié sur Pages ni utilisable hors session.
6. **CSP** injectée au build (meta) + script API depuis `grist.numerique.gouv.fr` (sans `eval` ; `docs.getgrist.com` est en mode eval et casse sous CSP). `img-src` autorise aussi `https://api.dicebear.com` pour les avatars Pixelbot de l’annuaire Équipe (seed stocké dans `Equipe.Avatar` ; pas de clé, images SVG publiques).
