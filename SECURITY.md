...
3. **Allowlist** des `fetchTable` : uniquement via `fetchAllowlistedTable` (`src/security/fetchTableAllowlist.ts`) — refus runtime hors liste, pas d’ID libre côté UI. Tables lazy (`Realise`, missions, équipe, produits) : allowlist **et** fetch uniquement sur `/bdc/:id` (Dépenses) ou `/missions` (liste + fiche) — pas au boot.
4. **Écriture métier** : interdite dans les hooks PA/BDC/Missions (pas de create/update `Realise`, etc.). **Exceptions bornées** : create sur `Retours` ; create/destroy sur `Feedback_Identite` (sonde `user.Name` / `user.Email`, car `/api/profile/user` + jeton doc renvoie souvent Anonymous). Garde [`writeTableAllowlist.ts`](src/security/writeTableAllowlist.ts). ACL Grist Create vs Update sur `Retours` = HITL (voir [`docs/fonctionnel/feedback/`](docs/fonctionnel/feedback/)).
5. **Téléchargement attachments / lecture BDC / Realise / profil feedback** : `docApi.getAccessToken({ readOnly: true })` → REST `?auth=` (jeton **court**, scoped **document + utilisateur** connecté). Identité feedback : priorité sonde table `Feedback_Identite` (plugin API) ; fallback `GET /api/profile/user?auth=…` (souvent Anonymous — ignoré). **Pas** de `VITE_GRIST_API_KEY`. Le jeton n’existe que pendant une session iframe Grist authentifiée — il n’est **pas** publié sur Pages ni utilisable hors session.
