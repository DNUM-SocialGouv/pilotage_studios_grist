...
3. **Allowlist** des `fetchTable` (pas d’ID de table libre côté UI).
4. **Lecture seule métier** V1 : pas d’API d’écriture dans les hooks.
5. **CSP** injectée au build (meta) : limite scripts / connect.
6. **Pas de source maps** en production.
7. **Repo privé** + revue PR / protection de `main` recommandées (org).
