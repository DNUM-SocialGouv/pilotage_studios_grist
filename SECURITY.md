...
3. **Allowlist** des `fetchTable` : uniquement via `fetchAllowlistedTable` (`src/security/fetchTableAllowlist.ts`) — refus runtime hors liste, pas d’ID libre côté UI. Tables lazy (`Realise`, missions, équipe, produits) : allowlist **et** fetch sur `/bdc/:id` (Dépenses), `/missions` (liste + fiche), `/cra` (liste lecture), `/outils/recap-porteurs` (récap porteurs), **ou** panneau feedback (select auteur → `Equipe` uniquement) — pas au boot. `Acl_profil` : lecture pour filtrer la nav / gardes de route (couche 5) — **pas** `Droits_pages` (table admin Owner/Admin only). `Retours` : lecture pour la colonne Feedback de l’accueil (liste telle quelle ; create reste via write allowlist).
