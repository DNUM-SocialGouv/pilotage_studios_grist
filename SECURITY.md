...
3. **Allowlist** des `fetchTable` : uniquement via `fetchAllowlistedTable` (`src/security/fetchTableAllowlist.ts`) — refus runtime hors liste, pas d’ID libre côté UI. Tables lazy (`Realise`, missions, équipe, produits) : allowlist **et** fetch sur `/bdc/:id` (Dépenses), `/missions` (liste + fiche), `/cra` (liste lecture), `/outils/recap-porteurs` (récap porteurs), **ou** panneau feedback (select auteur → `Equipe` uniquement) — pas au boot PA. `Acl_profil` : lecture **après** boot PA pour filtrer la nav / gardes (couche 5). `Droits_pages` : lecture lazy **Admin** sur `/outils/droits-pages` (ACL Grist Owner/Admin déjà en place). `Retours` : lecture **après** boot PA pour la colonne Feedback de l’accueil (liste telle quelle ; create reste via write allowlist).
4. **Écriture métier** : uniquement via `grist.getTable` + garde [`writeTableAllowlist.ts`](src/security/writeTableAllowlist.ts) — **pas** de REST avec clé API, **pas** de delete widget. Exceptions : `Retours` (create feedback) ; `Missions` (create + update patch Nom / Produit / Statut via drawer) ; `Missions_enfants` (create + update via drawer prestation — `Titre_de_la_prestation`, intervenant, jours, statut, date de début ; type Freelance figé à la création ; pas de delete) ; `Acl_profil` (**create only**, fiche session auto si absente — `E_mail` = compte connecté ; pas d’update/delete) ; `Droits_pages` (**update only**, page Admin — cases `Page_*` ; `Page_accueil` forcé à vrai ; pas de create/delete de rôles). Tout le reste (dont `Realise`, delete) reste interdit. ACL Grist Create/Update = HITL (feedback + missions + Acl_profil + droits pages).
